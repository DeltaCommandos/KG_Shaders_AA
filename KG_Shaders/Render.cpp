...
{   
	...
	if (mode == Mode::BUTTERFLY)
	{
		...
		for (double i = 0; i < 1; i += delta)
		{
			...
			glTexCoord2d(1, i + delta);
			glVertex3d(1, i + delta, -(i + delta) * (i + delta));

			glTexCoord2d(0, i + delta);
			glVertex3d(0, i + delta, -(i + delta) * (i + delta));
		...
		glScaled(5, 5, 5);
		glBegin(GL_QUADS);
		for (double i = 1; i < 1.5; i += delta)
		{
			glTexCoord2d(0, i);
			glVertex3d(0, i, i * i-2);

			glTexCoord2d(1, i);
			glVertex3d(1, i, i * i-2);

			glTexCoord2d(1, i + delta);
			glVertex3d(1, i + delta, (i + delta) * (i + delta)-2);

			glTexCoord2d(0, i + delta);
			glVertex3d(0, i + delta, (i + delta) * (i + delta)-2);
